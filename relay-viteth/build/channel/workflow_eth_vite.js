...
exports.WorkflowEthVite = void 0;
const vitejs_1 = require("@vite/vitejs");
class WorkflowEthVite {
    ...
    async step1() {
        ...
        const { toHeight, inputs } = await this.channelEther.scanConfirmedInputs(info.height);
        if (!inputs) {
            return;
        }
        ...
        const destAddress = vitejs_1.wallet.getAddressFromOriginalAddress(input.event.dest.slice(2));
        console.log(destAddress);
        await this.channelVite.approveAndExecOutput(input.id, destAddress, input.event.value.toString());
        await this.channelEther.updateInfo("_confirmed", {
...
exports.WorkflowEthVite = WorkflowEthVite;
