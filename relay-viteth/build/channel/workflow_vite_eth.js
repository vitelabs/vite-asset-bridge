...
exports.WorkflowViteEth = void 0;
const channelVite_1 = require("./channelVite");
class WorkflowViteEth {
    constructor(channelVite, channelEther) {
    ...
    async step1() {
        ...
        const { toHeight, events } = await this.channelVite.scanInputEvents(info.height);
        ...
        const input = events[0];
        {
            ...
            const isConfirmed = await channelVite_1.confirmed(this.channelVite.viteProvider, input.hash);
            if (!isConfirmed) {
                return;
            }
        }
    ...
    async step2() {
        ...
        const provedEvents = await proved.events
            ...
            .filter(async (x) => {
            return await this.channelEther.isKeeper("0x" + x.event.id, "0x" + x.event.sigR, "0x" + x.event.sigS, x.event.sigV);
        });
        ...
        await this.channelEther.approveAndExecOutput("0x" + input.event.id, provedEvents
        ...
        }), "0x" + input.event.dest, input.event.value);
...
exports.WorkflowViteEth = WorkflowViteEth;
